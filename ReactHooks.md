...
vuex包括这几个部分， `module` ， `state` ， `mutation` ， `action` ， `getters` (计算属性)

module：vuex可以拆分出很多个模块，每个 `module` 包括 `state` ， `mutation` ， `action` ，然后再export出去，在 `store文件夹` ，统一把所有的 `module`  `import` 进来，包装成统一的一个vuex状态管理机。
...
action: 可以支持异步操作逻辑的，但是最后需要改变状态的话，需要通过commit(mutation，val)传给mutation改变相应的状态
