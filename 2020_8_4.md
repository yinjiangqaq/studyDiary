...
那异步非阻塞的I/O代表着，如果setTimeout执行的回调，时间到了之后，不过前面是不是有代码阻塞到了，都会把代码操作权抛出去给setTimeout回调.

 具体情况看下图，如果从浏览器事件循环的角度看，如果while循环存在阻塞，还能让setTimeout事件的回调5s之后执行吗，实现5s之后自动停止吗? 显然不行
